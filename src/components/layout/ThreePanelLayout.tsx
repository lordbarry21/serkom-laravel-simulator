...
export function ThreePanelLayout() {
  const { activeRightTab, setRightTab, clearTerminal } = useSimulatorStore();

  return (
    <div className="flex-1 flex flex-col md:flex-row overflow-hidden relative">
      {/* Panel 1: Left Guide Panel */}
      ...
      {/* Panel 3: Right Panel - Terminal & Preview */}
      <section className="w-full md:w-[36%] lg:w-[34%] xl:w-[34%] h-1/3 md:h-full shrink-0 flex flex-col bg-[#0c0c0e] border-l border-zinc-800 overflow-hidden">
        ...
        <div className="h-9 px-3 bg-[#141416] border-b border-zinc-800 flex items-center justify-between select-none shrink-0">
          <div className="flex items-center gap-1">
            <button
              onClick={() => setRightTab('terminal')}
              className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium transition ${
                activeRightTab === 'terminal'
                  ? 'bg-zinc-800 text-sky-400 border border-zinc-700/80 shadow-sm'
          ...
          <div className="flex items-center gap-2">
            {activeRightTab === 'terminal' ? (
              <button
                ...
                className="text-[11px] text-zinc-400 hover:text-zinc-200 flex items-center gap-1 transition px-2 py-0.5 rounded hover:bg-zinc-800"
                title="Bersihkan layar (cls / clear)"
              >
